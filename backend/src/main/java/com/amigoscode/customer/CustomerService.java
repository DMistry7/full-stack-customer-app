...
public class CustomerService {
    ...
    private final S3Service s3Service; // One from main
    ...
    public void uploadCustomerProfileImage(Integer customerId,
                                           MultipartFile file) {
        checkIfCustomerExistsOrThrow(customerId);
        String profileImageId = UUID.randomUUID().toString(); //unique id for file name
        try {
            s3Service.putObject(
                    s3Buckets.getCustomer(),
                    "profile-images/%s/%s".formatted(customerId, profileImageId), //format puts custoemr id and uuid in  %s spots
                    file.getBytes()
            );
        } catch (IOException e) {
